...
TOOLS = [
    {
        ...
        "function": {
            ...
            "description": (
                "사업부/부서/개인 단위로 근무시간(실근로시간 또는 체류시간) '집계 수치'를 채팅으로 조회한다. "
                "period_value로 특정 시점(예: '2026-03')만 콕 집어 볼 수 있고, top_n을 주면 그 시점 기준 상위 N개만 "
                "정렬해서 돌려준다. '상위 5개 부서가 어디야', '가장 많이 일한 사업부는' 처럼 채팅으로 답만 받으면 "
                "되는 랭킹/집계 질문에 쓴다 — 직접 정렬하려 하지 말고 항상 top_n 파라미터로 서버가 정렬한 결과를 받아라. "
                "주의: 사용자가 '뽑아줘/추출해줘/엑셀로 줘'처럼 파일 자체를 원하면 이 도구가 아니라 "
                "export_worktime_detail을 써라 — 이 도구가 만드는 다운로드는 요약 한 장짜리라 표준 양식과 다르다."
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "지정한 조건에 해당하는 인원 '전체'의 일별/월별 근무시간을 사내 표준 엑셀 양식(일별 근무시간 시트 + "
                "월별 근무시간 시트, 근무시간류는 [h]:mm:ss로 표시)으로 추출할 파일을 준비한다. "
                "get_hours_summary가 만드는 요약 엑셀(사업부/부서 단위 집계치 한 장)과는 전혀 다른, "
                "개인별 출퇴근 원본 수준의 표준 양식이다. "
                "'뽑아줘' / '추출해줘' / '엑셀로 줘' / '파일로 줘' 라는 단어가 조금이라도 들어간 근무시간 관련 요청은 "
                "무조건 이 도구를 써라 — '근로시간 상위 N개 부서 뽑아줘'도 순위만 답하지 말고 이 도구를 "
                "top_n=N으로 호출해서 그 부서들 소속 인원 전체의 일별/월별 근무시간 파일을 준비해야 한다. "
...
def _build_system_prompt(context: dict) -> str:
    lines = [
        ...
        "- 사용자 메시지에 '뽑아줘'/'추출해줘'/'엑셀로'/'파일로' 라는 표현이 근무시간 얘기와 같이 나오면, "
        "그 안에 '상위 N개 부서' 같은 랭킹 조건이 섞여 있어도 절대 get_hours_summary로 순위만 답하고 끝내지 마라 — "
        "반드시 export_worktime_detail을 호출해서(top_n 등 조건 그대로 넘겨서) 표준 양식 엑셀 파일을 준비해라. "
        "이 둘은 서로 다른 파일이다: get_hours_summary의 다운로드는 부서별 집계 요약 한 장, "
        "export_worktime_detail은 개인별 일별/월별 근무시간 원본 양식.",
